...
window.get = function(name, def) { return window.localStorage.getItem(name) || def; };
...
$('#chat1').prepend(`<table id="script_options_table" class="hrpg_plus_table"><tbody><tr><td id="hrpg_options_title" align="center"><b>HRPG+ options [<a href="javascript:toggleHrpgOptions()">${get('optionsVisibility')=='true'?'-':'+'}</a>]</b></td></tr><tr><td id="script_options_td"><table id="script_options" class="hrpg_plus_table"/></td></tr></tbody></table>`);
$('#script_options').html(`<tr><td>Volume: 
<input oninput="maxi=document.getElementById('volume_num').value=event.target.value" id="volume_range" type="range" min="0" max="100" value="${get('volume', 100)}">
<input oninput="maxi=document.getElementById('volume_range').value=event.target.value" id="volume_num" type="number" min="0" max="100" value="${get('volume', 100)}"></td></tr>
...
<tr><td><table class="hrpg_plus_table"><tbody>
<tr><td>Nugget's Alerts: </td>
<td>${checkboxHtml('riftAlerts')} Rift™ </td>
<td>${checkboxHtml('riftTabTitle')} Title </td>
<td>${checkboxHtml('pmAlerts')} PM™ </td>
<td>${checkboxHtml('pmTabTitle')} Title </td>
...
$('#script_options_table').prepend('<table id="timers" class="hrpg_plus_table"><tbody><tr><td>Double: </td><td id="double_left"></td><td>Haste: </td><td id="haste_left"></td><td>Next scheduled rift: </td><td id="time_to_rift"></td></tr></tbody></table>');
...
window.command = {
  '': 'Posts <message> to main chat.',
  M: 'Posts <message> to main chat.',
  c: 'Posts <message> to clan chat.',
  t: 'Posts <message> to trade chat.',
  m: 'Posts <message> to <name>.',
  double: 'Shows length of double and haste remaining.',
  haste: 'Shows length of double and haste remaining.',
  commands: 'Shows a list of commands.',
  command: 'Shows information about <commandname>.',
  profile: 'Shows profile of <name>.',
  who: 'Shows basic information of <name>.',
  misc: 'Shows battles and quests of <name>.',
  crafting: 'Shows crafting levels of <name>.',
  gathering: 'Shows gathering levels of <name>.',
  stats: 'Shows your stats.',
  skills: 'Shows your skills.',
  ref: 'Shows your referral link and your referrals.'
};
...
window._Detab = function(text) {
  return text.replace(/\t(?=\t)/g,"    ")
    ...
    .replace(/~B(.+?)~A/g, function(wholeMatch,m1,m2) {
      return m1 + " ".repeat(4 - m1.length % 4); })
    .replace(/~A/g,"    ")
    .replace(/~B/g,"");
};
      ...
      data.c.forEach(function(item, i) {
        if($.inArray(data.c[i].uid, ignored) < 0) {
          ...
          if(c.type == 1) { $class = 'main'; msg = '<span class="time">['+c.time+']</span> '+genrank(c.nrank, c.prank)+' '+genStatus(c.status)+'<a href="javascript:m(\''+c.cname+'\');">'+c.cname+'</a>: '+c.message+'</span>';
            if($('#pingAlerts').is(':checked') && c.message.includes(yourName)){playAudio('bell');}
            if($('#pingTabTitle').is(':checked') && c.message.includes(yourName)){ document.title = 'Your name has been mentioned.'; tabTitleChanged = true; }}
            ...
            $('#info').html(`<tr><td>${c.message.slice(0,c.message.indexOf('Berserk')).replace(/: /g,':</td><td>').replace(/<br \/>/g,'</td></tr><tr><td>').replace('You have referred the following players:</td><td>', 'You have referred the following players:<br/>')}${ c.message.slice(c.message.indexOf('Berserk'))}</td></tr>`); return; }
            ...
            if($('#riftAlerts').is(':checked') && c.message.includes('A Rift has opened!')) {playAudio('rumble');}
            ...
            if($('#riftTabTitle').is(':checked') && c.message.includes('A Rift has opened!')) {document.title = 'A Rift has opened.'; tabTitleChanged = true;}
            ...
            if($('#pmAlerts').is(':checked')) {playAudio('message');}
            if($('#pmTabTitle').is(':checked')) {document.title = 'You have a new message.'; tabTitleChanged = true;}}
          ...
          if(chat_table == 10) {
            ...
            hideableMessageTypes.forEach(function(type_name, j) { if(!messageTypesVisible[j] && type_name.includes($class)) { $(`#ct1_tr${chatid1-1}`).hide(); } } );
